...
spark = SparkSession.builder.appName("customer-stedi").getOrCreate()
...
emailAndBirthDayFieldStreamingDF= emailAndBirthDayStreamingDF.select("email", split(emailAndBirthDayStreamingDF.birthDay,"-").getItem(0).alias("birthYear"))
...
stediStreamingDF.withColumn("value",from_json("value",stediEventsSchema))\
        .select(col('value.*')) \
...
customerStediDF=emailAndBirthDayFieldStreamingDF.join(customerRiskStreamingDF, expr("""
    email = customer
"""                                                                                 
))
...
customerStediDF.selectExpr("cast(customer as string) as key", "to_json(struct(*)) as value") \
    .writeStream \
    .format("kafka")\
    .option("kafka.bootstrap.servers", "localhost:9092")\
    .option("topic", "customer-stedi")\
